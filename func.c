...
int AddPerson(PersonList *PersonList)
{
    ...
    scanf("%s",PersonList->person[PersonList->length].name);
    while (1)
    {
        printf("%s%s%s",language[2],PersonList->person[PersonList->length].name,language[3]);    //"请输入" "的性别(M/W)："
        scanf(" %s",input);
        if (checkPersonSex(input[0]))
        {
            PersonList->person[PersonList->length].sex = toupper(input[0]);
            break;
        }
        printf("%s\n",language[28]);
    }
    while (1)
    {
        printf("%s%s%s",language[2],PersonList->person[PersonList->length].name,language[4]);     //"请输入" "的电话号码："
        scanf(" %s",input);
        if (checkPersonNumber(input))
        {
            strcpy(PersonList->person[PersonList->length].phoneNumber,input);
            break;
        }
        printf("%s\n",language[28]);
    }
    printf("%s%s%s",language[2],PersonList->person[PersonList->length].name,language[5]);     //"请输入" "的邮箱地址："
    scanf(" %s",PersonList->person[PersonList->length].email);
    while (1)
    {
        printf("%s%s%s",language[2],PersonList->person[PersonList->length].name,language[6]);     //"请输入" "的邮编："
        scanf(" %s",input);
        if (checkPersonNumber(input))
        {
            PersonList->person[PersonList->length].postCode = atoi(input);
            break;
        }
        printf("%s\n",language[28]);
    }
    printf("%s%s%s",language[2],PersonList->person[PersonList->length].name,language[7]);     //"请输入" "的地址："
    scanf(" %s",PersonList->person[PersonList->length].address);
    while (1)
    {
        printf("%s%s%s",language[8],PersonList->person[PersonList->length].name,language[9]);     //"是否把" "设为特别关心(Y/n)："
        scanf(" %s",input);
        if (checkPersonLike(input[0]))
        {
            PersonList->person[PersonList->length].like = (tolower(input[0]) == 'y') ? 1 : 0;      //1表示特别关心，0反之
            break;
        }
        printf("%s\n",language[28]);
    }
